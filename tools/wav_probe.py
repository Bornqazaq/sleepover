#!/usr/bin/env python3
"""Замер WAV без ffmpeg — проверка поставки звука.

Что делает и чего не делает. Он **меряет**, **срезает тишину по краям** и
**выравнивает пик** — и всё. Ни склейки петли, ни перекодирования, ни
ресемплинга. Разбор PCM идёт стандартной библиотекой, без единой зависимости:
на части машин ffmpeg в PATH нет (проверено 04.09 на ASRock).

Зачем он нужен сейчас, когда звук приходит готовым. Поставка приходит
нормализованной, и трогать её файлы не надо — паспорт говорит об этом прямо.
Но пачку стоит **проверить**: клиппинг, тишина вместо звука и полсекунды
пустоты перед атакой видны замером и не видны глазом. Клип с пустым началом
в игре звучит уже после события: створки пошли, а удар придёт через кадр.

Нашлось такое — это повод вернуть файл поставщику, а не чинить у себя.
Срез и выравнивание оставлены на крайний случай, когда ждать переделки дольше,
чем поправить.
"""
