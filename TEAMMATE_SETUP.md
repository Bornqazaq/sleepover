...
1. Сначала прочитай `CLAUDE.md` (корень) и `igruha/CLAUDE.md` — процесс и
   ...
   статус проекта, короткий и читается целиком. История прошлых проходов лежит
   отдельно в `docs/history/` и целиком не нужна: ищи по `docs/history/README.md`.
...
материалов при импорте даже без сохранения сцены (разбор — раздел 3.94
в `docs/history/STATE-archive.md`, номер строки смотреть в `docs/history/README.md`).
...
`tools/autorun-*.sh` и `tools/autorun-report.sh` (сличение логов), серия катки —
`tools/autorun-series.sh`. Грабли стенда — в `STATE.md`, раздел 8; подробные
разборы прогонов — в архиве `docs/history/`, поиск по указателю `README.md`.
