...
установка через npm). Прогон вскрыл четыре места, где инструкция обещала больше,
чем умеет: ярлык автозагрузки агенту недоступен при любых правах (§6.1),
`AutoLogonSID` может указывать на чужую учётку (§6.2), правила `.gitignore` для
`settings.local.json` не существовало (§7), а `remote-sessions.ps1` не находил
`claude.exe` после npm-установки (§11). Все четыре исправлены.
