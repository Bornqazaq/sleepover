...
namespace Igruha.EditorTools
{
    ...
    internal static class SyntyPalette
    {
        ...
        private static bool TryAccumulate(Mesh mesh, int subMesh, Material source, ref Vector3 sum, ref float weight)
        {
            ...
            // ⚠️ Здесь стоял ранний выход по mesh.isReadable — и он отменял
            // замер целиком. У импортёра Synty Read/Write выключен, поэтому
            // isReadable у всех мешей паков false <b>всегда</b>, а не только
            // после выгрузки. В редакторе при этом данные доступны: замер
            // 04.09 на SM_Bld_Rail_01 вернул 354 UV, 564 индекса и осмысленное
            // среднее #2A2C2F. То есть проверка отсекала не сломанный случай,
            // а рабочий, и все игры молча садились на сохранённые числа.
            //
            // Настоящий сломанный случай — меш, у которого Unity уже выгрузил
            // вершины: тогда массивы приходят пустыми, и это ловят проверки
            // ниже. Ими и ограничиваемся.
            //
            // Включать Read/Write по-прежнему нельзя: это правка настроек
            // импорта в чужом паке и удвоение памяти под каждый меш проекта.
