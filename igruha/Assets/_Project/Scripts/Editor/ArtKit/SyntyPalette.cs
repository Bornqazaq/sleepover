...
namespace Igruha.EditorTools
{
    ...
    internal static class SyntyPalette
    {
        ...
        private static bool TryAccumulate(Mesh mesh, int subMesh, Material source, ref Vector3 sum, ref float weight)
        {
            ...
            // Меш пака может быть закрыт на чтение: у импортёра Synty
            // Read/Write выключен, и Unity держит вершины только до первой
            // выгрузки. Обращение к закрытому мешу не бросает исключение —
            // оно печатает свою ошибку в консоль и возвращает пустой массив,
            // то есть замер молча провалился бы, оставив за собой две красные
            // строки на каждый предмет. Проверка возвращает честное «не смог»
            // и не пачкает консоль; вызывающий берёт сохранённое значение.
            //
            // Включать Read/Write нельзя: это правка настроек импорта в чужом
            // паке и удвоение памяти под каждый меш проекта.
            if (!mesh.isReadable)
            {
                return false;
            }
