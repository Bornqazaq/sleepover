...
namespace Igruha.EditorTools
{
    ...
    internal static class HoleInWallSilhouetteBaker
    {
        ...
        ///
        /// ⚠️ <b>Это же число — допуск попадания</b> (<c>CutoutShapes.Tolerance</c>).
        /// Так и должно быть: на смещении больше запаса кожа уходит в сплошную
        /// плиту, и засчитывать такой проход — показывать проход сквозь стену.
        /// Менять его здесь значит менять сложность игры.
        ///
        /// 0.15 м — верх диапазона, заданного геймдизайнером (0.10–0.15).
        /// Меньше нельзя: допуск и так закрывает воронка, а запас ниже 0.10
        /// съедает дрожь позы (около двух сантиметров на кисти). Больше нельзя:
        /// на 0.25 м подмышечный просвет «Титаника» затягивается, и силуэт
        /// перестаёт читаться позой — ради чего всё и делалось.
        /// </summary>
