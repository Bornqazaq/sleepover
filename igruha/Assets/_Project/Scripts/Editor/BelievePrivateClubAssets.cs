...
namespace Igruha.EditorTools
{
    ...
    internal static class BelievePrivateClubAssets
    {
        ...
        internal static void Prepare()
        {
            ...
            // Значение проверяется двумя кадрами сразу, и они тянут в разные
            // стороны. В широком кадре зала столб хочется плотнее; в кадре
            // сидящего камера стоит внутри объёма, и на .042 тёплая дымка
            // затягивала весь экран, поднимая фон за лицом соперника из
            // чёрного в светло-коричневый. Решает кадр сидящего: игра про лицо.
            volume.SetColor("_BaseColor", new Color(.98f, .74f, .46f, .020f));
