...
Причина: список сетевых префабов у главного редактора и у виртуального игрока
разошёлся. Так бывает после того, как префабу добавили `NetworkObject` или
изменили `DefaultNetworkPrefabs` — у клона своя `Library`, и он может остаться
на старом импорте. Перезапуск сценария это не всегда лечит.

Лечение: закрыть сценарий и удалить `Library/VP/<id>/Library` — клон соберёт
импорт заново. Папка одноразовая, в гите её нет.
