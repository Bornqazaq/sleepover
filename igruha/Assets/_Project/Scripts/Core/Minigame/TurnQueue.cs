...
namespace Igruha.Core.Minigame
{
    ...
    public sealed class TurnQueue
    {
        ...
        /// <summary>
        /// Порядок целиком, только для чтения. Нужен интерфейсу: показать,
        /// кто ходит следующим, — а это общеизвестно, очередь объявлена всем
        /// в начале раунда.
        /// </summary>
        ...
        /// <summary>
        /// Принять готовый порядок, объявленный сервером. Тасовать нечего:
        /// очередь уже составлена и приехала списком.
        ///
        /// Порядок объявляется <b>списком, а не сидом тасования</b>. Сид сошёлся
        /// бы в тот же порядок только при точно совпадающем составе на обеих
        /// машинах, а список сходится всегда. Для игры, где сид — секрет
        /// («Рейс на память»), это заодно означает, что серверу не приходится
        /// объявлять вообще ни одного сида.
        ///
        /// Указатель текущего и список выбывших не трогаются: это знание
        /// сервера. Кто ходит сейчас, машине игрока сообщает правило игры
        /// своим каналом.
        /// </summary>
        public void ApplyOrder(IReadOnlyList<int> ids)
        {
            Clear();
