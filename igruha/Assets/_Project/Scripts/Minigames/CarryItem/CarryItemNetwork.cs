...
namespace Igruha.Minigames.CarryItem
{
    ...
    public sealed class CarryItemNetwork : NetworkBehaviour
    {
        ...
        /// <summary>
        /// Состав, под который состояние уже разобрано. −1 — ещё ни разу.
        ///
        /// Порядок спавна этого объекта и старта мини-игры ничем не связан:
        /// состав вполне может приехать раньше, чем контроллер получит ростер
        /// сессии, — и разобрать его тогда некуда. Второй раз само оно не
        /// приедет (список больше не меняется), и без этой сверки клиент
        /// досидел бы раунд, не зная <b>ни одной команды</b>: чужой ему был бы
        /// каждый, включая себя, — а значит ни своего штабеля, ни своей ручки,
        /// ни своей половины полосы прогресса.
        ///
        /// Ровно та же страховка у «Рейса на память» и по той же причине.
        /// </summary>
        ...
        private void LateUpdate()
        {
            ...
            // Ростер сессии мог собраться уже после того, как состав приехал:
            // тогда разбираем заново, иначе клиент досидит раунд без команд.
