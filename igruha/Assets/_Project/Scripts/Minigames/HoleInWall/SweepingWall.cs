...
namespace Igruha.Minigames.HoleInWall
{
    ...
    /// только того, кто уже провалился, а прошедшему их снимает машина,
    /// которая ведёт его тело (<see cref="DisableCollision"/>). Разбор —
    /// архив STATE, раздел 3.52, и IGR-594.
    ///
    ...
    public sealed class SweepingWall : MonoBehaviour
    {
        ...
        /// на всех машинах. Звук фазы 5 гонит по нему тон и громкость
        ...
        /// <summary>Плиты сейчас твёрдые. Снимаются один раз за проезд — см. <see cref="DisableCollision"/>.</summary>
        ...
        /// <summary>
        /// Снять с плит столкновения до конца прохода. <b>У каждой машины своё
        /// решение</b>, и принимает его <c>HoleInWallMinigame</c> в двух случаях:
        ///
        /// <list type="bullet">
        /// <item>видимая грань дошла до тела, которое ведёт эта машина, и оно
        /// стоит в своём вырезе — плиты подошли бы к нему следующими;</item>
        /// <item>пришёл вердикт «прошли».</item>
        /// </list>
        ///
        /// ⚠️ Раньше плиты снимал только вердикт — и только у сервера. Клиент
        /// видит стену позже сервера на полпути связи (его часы — это
        /// <c>ServerTime</c>, время уже полученных пакетов), и у него плиты
        /// приезжали к прошедшему твёрдыми: прошедшего, который ещё
        /// подстраивался, стена могла столкнуть в воду уже после засчитанного
        /// очка. Вероятная причина «напарник упал, а второму засчитали»
        /// с плейтеста 20.09: механизм подтверждён стендом (снос у клиента
        /// приходил раньше его стены), само сталкивание — нет, болванка после
        /// линии не шевелится.
        /// Провалившимся плиты остаются: их стена и обязана ударить.
        /// </summary>
