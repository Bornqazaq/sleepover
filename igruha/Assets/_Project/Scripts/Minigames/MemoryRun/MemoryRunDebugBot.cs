...
namespace Igruha.Minigames.MemoryRun
{
    ...
    /// (спека 10.3). Единственное исключение — процесс, запущенный
    /// с <c>--bot</c>: за такой клавиатурой человека нет вовсе, и там болванка
    /// ведёт <b>только персонажа своей машины</b>, чужими не рулит никогда.
    /// Признак болванки «этой машиной не управляется» у сервера выглядит
    /// одинаково для любого подключившегося клиента — на этом уже дважды
    /// обжигались, в «Экзамене» и в «Верю / не верю».
    ///
    ...
    public sealed class MemoryRunDebugBot : MonoBehaviour
    {
        ...
        [Tooltip("Отдать болванке и персонажа этой машины — тогда соло-прогон идёт целиком сам, без рук. Только для одиночного прогона: в сетевой катке своего персонажа отдаёт аргумент запуска --bot, а без него погонщик выключается совсем")]
        ...
        private void Update()
        {
            ...
            if (networked)
            {
                // Стенд из восьми процессов — единственное исключение, и оно
                // не по признаку, а по аргументу запуска. Процесс с --bot
                // человеком не управляется вовсе: за клавиатурой там никого
                // нет. Тот же приём у «Экзамена» и «Верю / не верю».
            ...
            // 🔴 Главная страховка сетевой катки: в ней болванка ведёт только
            // персонажа СВОЕЙ машины. Чужая копия выглядит у сервера ровно как
            // манекен — «этой машиной не управляется», — и без этой строки
            // погонщик прыгал бы за живого человека, который стоит и вспоминает
            // маршрут. Тот же класс ошибок ловили в «Экзамене» (STATE 3.17)
            // и в «Верю / не верю» (STATE 3.18).
            if (networked && !reader.LocallyControlled)
            {
                return;
            }
        ...
        /// <summary>
        /// Отдать болванке персонажа этой машины, чтобы прогон шёл сам.
        ///
        /// Иначе проверка упирается в человека за клавиатурой: первый персонаж
        /// локально управляем, <c>DriveMove</c> ему запрещён, и его ход каждый
        /// круг просто истекает по таймеру. Дёргать автопилот снаружи, из
        /// play-режима, не годится — мост на выполнении кода ставит паузу,
        /// и игра перестаёт тикать вовсе.
        ///
        /// Берётся <b>только локально управляемый</b> ридер, поэтому в сетевой
        /// катке взводится ровно один персонаж — свой. Чужие копии сюда не
        /// попадают ни при каком составе.
        /// </summary>
