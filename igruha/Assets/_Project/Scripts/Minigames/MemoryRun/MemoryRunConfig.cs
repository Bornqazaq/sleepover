...
namespace Igruha.Minigames.MemoryRun
{
    ...
    public sealed class MemoryRunConfig : ScriptableObject
    {
        ...
        public Vector3 CellCenter(int step, int lane) => new Vector3(LaneX(lane), PlateSurfaceY, StepZ(step));

        /// <summary>
        /// Высота настила, по которому ходят: верхняя грань плиты и она же
        /// поверхность выходной площадки. Ноль — и это не совпадение:
        /// построитель арены сажает плиты центром на <c>−толщина/2</c>
        /// именно затем, чтобы все переходы между рядами шли строго
        /// по горизонтали и были одинаковыми.
        ///
        /// Вынесено в свойство, потому что по этой отметке сервер решает,
        /// стоит ли игрок на плите. Число, оставленное литералом в двух
        /// местах, однажды разъедется с геометрией молча.
        /// </summary>
        public float PlateSurfaceY => 0f;
