...
Shader "Igruha/HoleInWall/Pavilion Water"
{
    Properties
    {
        _PlanarReflection("Planar reflection",2D)="black" {}
        _PlanarAvailable("Planar available",Float)=0
        _PavilionEnvironment("Pavilion reflection", Cube) = "black" {}
        _ReflectionStrength("Room reflection", Range(0,1)) = .5
        _ProbePosition("Probe position", Vector)=(0,0,0,0)
        _RoomMin("Room minimum", Vector)=(-30,-3,-20,0)
        _RoomMax("Room maximum", Vector)=(30,24,40,0)
        [Header(Cvet)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Blend Off
            ZWrite Off
            // Cull Off, а не Back: упавший смотрит на поверхность СНИЗУ.
            // С отсечением задних граней её оттуда не видно вовсе, и кадр
            // под водой становился пустым провалом — по нему нельзя было
            // понять даже, что ты в воде. Одна грань стоит того.
            Cull Off
            ...
            float4x4 _PlanarVP;
            TEXTURECUBE(_PavilionEnvironment); SAMPLER(sampler_PavilionEnvironment);
            CBUFFER_START(UnityPerMaterial)
                ...
                float _PlanarAvailable;
                float4 _ProbePosition, _RoomMin, _RoomMax;
            CBUFFER_END
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float3 rb=lerp(_RoomMin.xyz,_RoomMax.xyz,step(0,reflected));
                float3 distanceToBox=(rb-input.positionWS)/(sign(reflected)*max(abs(reflected),.0001));
                float rayDistance=min(distanceToBox.x,min(distanceToBox.y,distanceToBox.z));
                float3 cubeDirection=input.positionWS+reflected*rayDistance-_ProbePosition.xyz;
                half3 room=SAMPLE_TEXTURECUBE_LOD(_PavilionEnvironment,sampler_PavilionEnvironment,cubeDirection,1.2).rgb;
                if(_PlanarAvailable>.5) {
                    float4 projected=mul(_PlanarVP,float4(input.positionWS,1));
                    float2 reflectedUV=projected.xy/projected.w*.5+.5;
                    reflectedUV+=normalWS.xz*.012;
                    room=SAMPLE_TEXTURE2D(_PlanarReflection,sampler_PlanarReflection,saturate(reflectedUV)).rgb;
                }
                float reflectAmount=(.08+.72*pow(1-saturate(dot(normalWS,viewDirWS)),2))*_ReflectionStrength;
                color=lerp(color,room,reflectAmount*(1-underside)*nearSurface);
